...
# we only have the tuned CPU version, that does not accept jax arrays
...
for i in range(2):
    ...
    t1 = time.time()
    # assuming we only had the jax array, we must explicitly convert it first
    c2 = center_distance_matrix_skbio(numpy.array(jmat_small))
    # JAX will accept a numpy array and auto-convert it, if needed
    ...
    t1 = time.time()
    # assuming we only had the jax array, we must explicitly convert it first
    c2 = center_distance_matrix_skbio(numpy.array(jmat_med))
    # JAX will accept a numpy array and auto-convert it, if needed
    q2,r2 = jax.numpy.linalg.qr(c2)
    # JAX is async... we wait before the timing function
    q2.block_until_ready()
    ...
    t1 = time.time()
    # assuming we only had the jax array, we must explicitly convert it first
    c2 = center_distance_matrix_skbio(numpy.array(jmat))
    # JAX will accept a numpy array and auto-convert it, if needed
    q2,r2 = jax.numpy.linalg.qr(c2)
    # JAX is async... we wait before the timing function
    q2.block_until_ready()
