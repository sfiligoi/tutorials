...
Here we simulate a 3-file setup,
...
Main:
  cpp_ext.cpp
External functions (not GPU aware), mimicking a library:
  cpp_ext_inlines.hpp
  cpp_ext_func.cpp
Wrapper to add GPU compilation:
  cpp_ext_func_omp_wrap.cpp

and then try to build and execute the compiled apps with
make cpp_ext
# Force the use of the 2nd APU on the node
taskset -c 24-47,120-143 ./cpp_ext_cpu
export ROCR_VISIBLE_DEVICES=1
# Must explicity enable HSA_XNACK to get APU semantics
HSA_XNACK=1 taskset -c 24-47,120-143 ./cpp_ext_gpu

6) Using external functions - GPU aware
---------------------------------------
Not all code is typically in a single source file.
Keeping functions in separate files and compiling them 
independently is standard practive in non-trivial applications.

Here we simulate a 3-file setup, with all the code GPU aware.

Look inside the following files:
Main:
  omp_ext.cpp
External functions (GPU aware), mimicking a library:
  omp_ext_inlines.hpp
  omp_ext_func.cpp
